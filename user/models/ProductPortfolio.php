...
function productPortfolio_GetAllProduct($idCategory) {
    $sql = "select
     p.IdProduct,p.NameProducts,p.ImageProducts, p.PriceProducts, d.ProductDetails 
     from product p
    join details d on p.IdDetails = d.IdDetails 
    where p.StatusProducts = 0 and p.IdCategory = $idCategory limit 12 ";
...
function productPortfolio_GetAllProductAsRequested($dataPrice, $dataProduct, $idCategory){
    ...
    if($dataPrice != "" && $dataProduct === ""){
        ...
        $sql = "SELECT p.IdProduct, p.NameProducts, p.ImageProducts, p.PriceProducts, d.ProductDetails
        FROM product p
        JOIN details d ON p.IdDetails = d.IdDetails
        WHERE p.StatusProducts = 0
          AND p.IdCategory = $idCategory
          AND p.PriceProducts BETWEEN $priceStart AND $priceEnd
        LIMIT 12;
        ";
        ...
        from product p
       join details d on p.IdDetails = d.IdDetails 
       where p.StatusProducts = 0 and p.IdCategory = $dataProduct limit 12 ";
        ...
        from product p
        join details d on p.IdDetails = d.IdDetails 
        where p.StatusProducts = 0 and p.IdCategory = $dataProduct 
        PriceProducts between $priceStart and $priceEnd
        limit 12 ";
